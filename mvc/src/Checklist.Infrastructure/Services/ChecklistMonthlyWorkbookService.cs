...
public class ChecklistMonthlyWorkbookService
{
    ...
    public ChecklistMonthlyWorkbookService(IWebHostEnvironment environment)
    {
    ...
    public byte[] Generate(ChecklistMonthlySnapshot snapshot)
    {
        ...
        if (snapshot.MonthlyClosureModel == MvcMonthlyClosureModel.None)
        {
    ...
    private static void FillWorkbook(string workbookPath, ChecklistMonthlySnapshot snapshot)
    {
        ...
        var sheetEntryPath = KeepOnlyCategorySheet(archive, snapshot.MonthlyClosureModel);
    ...
    private static string KeepOnlyCategorySheet(ZipArchive archive, MvcMonthlyClosureModel model)
    {
        var selectedSheetPath = model == MvcMonthlyClosureModel.ElectricForklift
        ...
        var selectedSheet = sheets.SingleOrDefault(sheet => string.Equals(GetWorksheetPath(workbookRelationships, sheet), selectedSheetPath, StringComparison.Ordinal))
        ...
        var removedSheets = sheets.Where(sheet => !ReferenceEquals(sheet, selectedSheet)).ToList();
        ...
        workbook.Root?.Element(Ns + "bookViews")?.Element(Ns + "workbookView")?.SetAttributeValue("activeTab", 0);
        workbook.Root?.Element(Ns + "bookViews")?.Element(Ns + "workbookView")?.SetAttributeValue("firstSheet", 0);
    ...
    private static void NormalizeDefinedNames(XDocument workbook, int selectedSheetIndex)
    {
        ...
        foreach (var definedName in definedNames.Elements(Ns + "definedName").ToList())
        {
    ...
    {
        SetCellText(worksheet, "J7", $"{snapshot.Month:00}/{snapshot.Year}");
        SetCellText(worksheet, "M7", snapshot.SectorName);
        SetCellText(worksheet, "AQ7", snapshot.EquipmentCode);
    }

    private static void WriteStatuses(XDocument worksheet, ChecklistMonthlySnapshot snapshot)
    {
        var rowsByOrder = snapshot.Rows.ToDictionary(x => x.Order);
        ...
        for (var day = 1; day <= 31; day++)
        {
            ...
            for (var index = 0; index < ItemRowStarts.Length; index++)
            {
                ...
                if (day <= snapshot.DaysInMonth
                    && rowsByOrder.TryGetValue(index + 1, out var row)
                    && day - 1 < row.ValuesByDay.Count)
                {
                    value = row.ValuesByDay[day - 1] ?? string.Empty;
                }
    ...
    {
        var days = snapshot.Days.ToDictionary(x => x.Day);

        for (var day = 1; day <= 31; day++)
        {
            var column = ToColumnName(11 + ((day - 1) * 2));
            var value = days.TryGetValue(day, out var item)
                ? $"{item.OperatorRegistration}{Environment.NewLine}{item.OperatorName}"
                : string.Empty;
    ...
    private static void WriteComments(XDocument worksheet, ChecklistMonthlySnapshot snapshot)
    {
        ...
        if (snapshot.Comments.Count > MaxComments)
        {
    ...
    {
        var root = worksheet.Root ?? throw new InvalidOperationException("Template mensal invalido.");
    ...
    {
        if (mergeCells.Elements(Ns + "mergeCell").Any(x => string.Equals((string?)x.Attribute("ref"), range, StringComparison.Ordinal)))
        {
            return;
        }
